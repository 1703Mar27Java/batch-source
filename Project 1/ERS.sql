...
/*******************************************************************************
   Create Procedures
*******************************************************************************/
CREATE OR REPLACE PROCEDURE SP_ERS_SUBMIT (AMOUNT IN NUMBER, DESCRIPTION IN VARCHAR, USERID IN NUMBER, R_TYPE IN VARCHAR)
IS
  TIME_NOW TIMESTAMP:=CURRENT_TIMESTAMP;
  ID_TYPE NUMBER;
  ID_STATUS NUMBER;
  RTYPE VARCHAR2(50);
BEGIN
  SELECT RT_ID INTO ID_TYPE FROM ERS_REIMBURSEMENT_TYPE WHERE R_TYPE = RTYPE;
  SELECT RS_ID INTO ID_STATUS FROM ERS_REIMBURSEMENT_STATUS WHERE RS_STATUS = 'Pending'; 
  INSERT INTO ERS_REIMBURSEMENTS (R_AMOUNT,R_DESCRIPTION, R_SUBMITTED, U_ID_AUTHOR, RT_TYPE, RS_STATUS )
                          VALUES (AMOUNT, DESCRIPTION, TIME_NOW, USERID, ID_TYPE, ID_STATUS);
END;

CREATE OR REPLACE PROCEDURE SP_ERS_RESOLVE (REIMB_ID IN NUMBER, STATUS IN VARCHAR, RESOLVERID IN NUMBER)
IS 
  STATUS_ID NUMBER;
  TIME_NOW TIMESTAMP:=CURRENT_TIMESTAMP; 
BEGIN
  SELECT RS_ID INTO STATUS_ID FROM ERS_REIMBURSEMENT_STATUS WHERE RS_STATUS = STATUS;
  UPDATE ERS_REIMBURSEMENTS SET RS_STATUS = STATUS_ID, R_RESOLVED = TIME_NOW, U_ID_RESOLVER = RESOLVERID WHERE ERS_REIMBURSEMENTS.R_ID = REIMB_ID;
END;
