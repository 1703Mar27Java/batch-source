...
public class BankAccountDaoImpl implements BankAccountDao {
	...
	//returns a list of all bank accounts under the current user
	public List<BankAccount> retrieveAccountsById(int userId){
		PreparedStatement pstmt = null;
		List<BankAccount> accounts = new ArrayList<>();
		try{
			Connection con = ConnectionUtil.getConnection();
			String sql = "SELECT * FROM BANKACCOUNT WHERE USERID IN '" + userId +"'";
			pstmt = con.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
				int acctId = rs.getInt("ACCOUNTID");
